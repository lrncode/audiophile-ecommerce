...
export default function FormElement({handleChange , error , data , label , name , placeholder , additionalClass= ''}) {
  ...
  return (
    ...
    <div className={`${error !== errorPlaceholder ? 'checkout-form-label error-text' : 'checkout-form-label'}`}><label htmlFor='fullName'>{label}</label><p className={`${ error !== errorPlaceholder ? 'error-text show' : 'error-text hide'}`}>{error}</p></div>
    <input id={name}
           value={data}
           name={name}
